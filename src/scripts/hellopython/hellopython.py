...
class HelloPlugin(Bhawk.PluginInterface, QtCore.QObject):
    buttons = {}
    ...
    def init(self, state, settingsPath):
        ...
        Bhawk.SideBarManager.addSidebar("hellopython-sidebar", self.sidebar)

        self.schemeHandler = HelloSchemeHandler()
        Bhawk.MainApplication.instance().networkManager().registerExtensionSchemeHandler("hello", self.schemeHandler)

        if state == Bhawk.PluginInterface.LateInitState:
            for window in Bhawk.MainApplication.instance().windows():
                self.mainWindowCreated(window)

    def unload(self):
        print("unload")

        Bhawk.SideBarManager.removeSidebar(self.sidebar)
...
Bhawk.registerPlugin(HelloPlugin())

class HelloSchemeHandler(Bhawk.ExtensionSchemeHandler):
