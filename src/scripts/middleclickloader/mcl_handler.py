...
class MCL_Handler(QtCore.QObject):
    ...
    def __init__(self, settingsPath, parent=None):
        ...
        self.settingsFile = settingsPath + "/plug-ins.ini"
    ...
    def mousePress(self, view, event):
        ...
        if event.buttons() == QtCore.Qt.MiddleButton:
            ...
            if selectionClipboard:
                ...
                if not isValid:
                    searchManager = Bhawk.MainApplication.instance().searchEnginesManager()
    ...
    def loadUrl(self, view, url):
        if self.loadMode == MCL_LoadMode.NEW_TAB:
            view.openUrlInNewTab(url, Bhawk.Qz.NT_NotSelectedTab)
        elif self.loadMode == MCL_LoadMode.CURRENT_TAB:
            view.load(url)
        elif self.loadMode == MCL_LoadMode.NEW_WINDOW:
            Bhawk.MainApplication.instance().createWindow(Bhawk.Qz.BW_NewWindow, url)
        else:
            return False
        return True
