...
void SpeedDial::loadSettings() {
    ...
    m_maxPagesInRow = settings.value("pagesrow", 7).toInt();
    m_sizeOfSpeedDials = settings.value("sdsize", 285).toInt();
    m_sdcentered = settings.value("sdcenter", true).toBool();
    settings.endGroup();
...
QString SpeedDial::initialScript()
{
    ...
    for (const Page &page : qAsConst(m_pages)) {
        ...
        const QByteArray urlData = page.url.toUtf8();
        const QString hexHash = QCryptographicHash::hash(urlData, QCryptographicHash::Md4).toHex();
        QString imgSource = m_thumbnailsDir + hexHash + ".png";
        ...
        QVariantMap map;
        map[QStringLiteral("url")] = page.url;
        map[QStringLiteral("title")] = page.title;
        map[QStringLiteral("img")] = imgSource;
        pages.append(map);
    }
...
void SpeedDial::changed(const QString &allPages)
{
    ...
    for (const QString &entry : entries) {
        ...
        if (page.url.endsWith(QLatin1Char('/')))
...
void SpeedDial::loadThumbnail(const QString &url, bool loadTitle)
{
    ...
    connect(thumbnailer, &PageThumbnailer::thumbnailCreated, this, &SpeedDial::thumbnailImg);
...
void SpeedDial::thumbnailImg(const QPixmap &pixmap) {
    ...
    const bool loadTitle = thumbnailer->loadTitle();
    QString title = thumbnailer->title();
    const QString url = thumbnailer->url().toString();
    const QByteArray urlData = url.toUtf8();
    const QString hexHash = QCryptographicHash::hash(urlData, QCryptographicHash::Md4).toHex();
    QString fileName = m_thumbnailsDir + hexHash + ".png";
    ...
    if (pixmap.isNull()) {
        ...
        if (!pixmap.save(fileName, "PNG")) {
            fileName = ":/html/broken-page.svg";
            qWarning() << "SpeedDial::thumbnailImg Cannot save thumbnail to " << fileName;
        }
