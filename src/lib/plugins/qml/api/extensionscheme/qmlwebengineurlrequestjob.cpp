...
void QmlWebEngineUrlRequestJob::reply(const QVariantMap &map)
{
    ...
    if (!map.contains(QStringLiteral("content")) || !map.contains(QStringLiteral("contentType"))) {
        qWarning() << "Unable to call" << __FUNCTION__ << ": invalid parameters";
        return;
    }
    QByteArray content = map.value(QStringLiteral("content")).toString().toUtf8();
    QByteArray contentType = map.value(QStringLiteral("contentType")).toString().toUtf8();
