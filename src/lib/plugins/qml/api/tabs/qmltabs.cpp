...
bool QmlTabs::setCurrentIndex(const QVariantMap &map)
{
    ...
    const int index = map.value(QStringLiteral("index")).toInt();

    BrowserWindow *const window = getWindow(map);
    if (!window) {
        return false;
    }
...
{
    BrowserWindow *const window = getWindow(windowId);
    if (!window) {
        return false;
    }
...
{
    if (!map.contains(QStringLiteral("from"))) {
        qWarning() << "Unable to move tab:" << "from not defined";
        return false;
    }
    if (!map.contains(QStringLiteral("to"))) {
        qWarning() << "Unable to move tab:" << "to not defined";
        return false;
    }

    const int from = map.value(QStringLiteral("from")).toInt();
    const int to = map.value(QStringLiteral("to")).toInt();
...
QmlTab *QmlTabs::get(const QVariantMap &map) const
{
    ...
    WebTab *const webTab = window->tabWidget()->webTab(index);
...
QList<QObject*> QmlTabs::getAll(const QVariantMap &map) const
{
    ...
    const bool withPinned = map.value(QStringLiteral("withPinned")).toBool();
...
{
    const int windowId = map.value(QStringLiteral("windowId"), -1).toInt();
...
BrowserWindow *QmlTabs::getWindow(int windowId) const
{
    ...
    QHash<BrowserWindow *, int> windowIdHash = QmlStaticData::instance().windowIdHash();
    for (QHash<BrowserWindow *, int>::const_iterator it = windowIdHash.cbegin(); it != windowIdHash.cend(); it++) {
...
void QmlTabs::windowCreated(BrowserWindow *window)
{
    ...
    connect(window->tabWidget(), &TabWidget::tabInserted, this, [this, windowId](int index){
        QVariantMap map;
        map.insert(QStringLiteral("windowId"), windowId);
