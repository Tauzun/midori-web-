...
QmlSettings::QmlSettings(QObject *parent)
    : QObject(parent)
{
    m_settingsPath = DataPaths::currentProfilePath() + QLatin1String("/plug-ins");
}
...
bool QmlSettings::setValue(const QVariantMap &map)
{
    ...
    if (!map.contains(QStringLiteral("key")) || !map.contains(QStringLiteral("value"))) {
...
QVariant QmlSettings::value(const QVariantMap &map)
{
    ...
    const QString key = map.value(QStringLiteral("key")).toString();
    const QVariant defaultValue = map.value(QStringLiteral("defaultValue"));
...
{
    m_settingsPath += QLatin1Char('/') + m_name + QLatin1String("/settings.ini");
