...
Plugins::Plugin QmlPlugin::loadPlugin(const QString &name)
{
    ...
    if (QFileInfo(name).isAbsolute()) {
        ...
        if (fullPath.isEmpty()) {
            qWarning() << "QML plug-in" << name << "not found";
    ...
    plugin.pluginId = QStringLiteral("qml:%1").arg(QFileInfo(name).fileName());
    plugin.pluginPath = fullPath;
    DesktopFile desktopFile(fullPath + QStringLiteral("/metadata.desktop"));
