...
#define INTERNAL_SERVER_ID QLatin1String("bhawk.internal")
    ...
    , m_askMasterPassword(false)
{
    ...
    if (!db.tables().contains(QLatin1String("autofill_encrypted"))) {
        db.exec(QStringLiteral("CREATE TABLE autofill_encrypted (data_encrypted TEXT, id INTEGER PRIMARY KEY,"
                "password_encrypted TEXT, server TEXT, username_encrypted TEXT, last_used NUMERIC)"));
        db.exec(QStringLiteral("CREATE INDEX autofillEncryptedServer ON autofill_encrypted(server ASC)"));
    }
}
...
QStringList DatabaseEncryptedPasswordBackend::getUsernames(const QUrl &url)
{
    ...
    query.prepare(QStringLiteral("SELECT username_encrypted FROM autofill_encrypted WHERE server=? ORDER BY last_used DESC"));
    query.addBindValue(PasswordManager::createHost(url));
    query.exec();

    QStringList list;
    while (query.next()) {
        list.append(QStringLiteral("Encrypted %1").arg(list.size() + 1));
    }
    return list;
}
...
QVector<PasswordEntry> DatabaseEncryptedPasswordBackend::getEntries(const QUrl &url)
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id, username_encrypted, password_encrypted, data_encrypted FROM autofill_encrypted "
...
QVector<PasswordEntry> DatabaseEncryptedPasswordBackend::getAllEntries()
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id, server, username_encrypted, password_encrypted, data_encrypted FROM autofill_encrypted"));
    query.exec();
...
void DatabaseEncryptedPasswordBackend::addEntry(const PasswordEntry &entry)
{
    ...
    if (entry.data.isEmpty()) {
        ...
        QSqlQuery query(SqlDatabase::instance()->database());
        query.prepare(QStringLiteral("SELECT username_encrypted FROM autofill_encrypted WHERE server=?"));
    ...
    if (hasPermission() && encryptPasswordEntry(encryptedEntry, &aesEncryptor)) {
        QSqlQuery query(SqlDatabase::instance()->database());
        query.prepare(QStringLiteral("INSERT INTO autofill_encrypted (server, data_encrypted, username_encrypted, password_encrypted, last_used) "
...
bool DatabaseEncryptedPasswordBackend::updateEntry(const PasswordEntry &entry)
{
    ...
    if (hasPermission() && encryptPasswordEntry(encryptedEntry, &aesEncryptor)) {
        ...
        // Data is empty only for HTTP/FTP authorization
        if (entry.data.isEmpty()) {
            query.prepare(QStringLiteral("UPDATE autofill_encrypted SET username_encrypted=?, password_encrypted=? WHERE server=?"));
            query.bindValue(0, encryptedEntry.username);
            query.bindValue(1, encryptedEntry.password);
            query.bindValue(2, encryptedEntry.host);
        }
        else {
            query.prepare(QStringLiteral("UPDATE autofill_encrypted SET data_encrypted=?, username_encrypted=?, password_encrypted=? WHERE id=?"));
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("UPDATE autofill_encrypted SET last_used=strftime('%s', 'now') WHERE id=?"));
...
void DatabaseEncryptedPasswordBackend::removeEntry(const PasswordEntry &entry)
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("DELETE FROM autofill_encrypted WHERE id=?"));
    query.addBindValue(entry.id);
    query.exec();
...
void DatabaseEncryptedPasswordBackend::removeAll()
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("DELETE FROM autofill_encrypted"));
    query.exec();
...
void DatabaseEncryptedPasswordBackend::encryptDataBaseTableOnFly(const QByteArray &decryptorPassword, const QByteArray &encryptorPassword)
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id, data_encrypted, password_encrypted, username_encrypted, server FROM autofill_encrypted"));
    query.exec();
    ...
    while (query.next()) {
        ...
        updateQuery.prepare(QStringLiteral("UPDATE autofill_encrypted SET data_encrypted = ?, password_encrypted = ?, username_encrypted = ? WHERE id = ?"));
...
QByteArray DatabaseEncryptedPasswordBackend::someDataFromDatabase()
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT password_encrypted, data_encrypted, username_encrypted FROM autofill_encrypted"));
    query.exec();
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id FROM autofill_encrypted WHERE server = ?"));
    ...
    if (!password.isEmpty()) {
        ...
        if (query.next()) {
            query.prepare(QStringLiteral("UPDATE autofill_encrypted SET password_encrypted = ? WHERE server=?"));
        }
        else {
            query.prepare(QStringLiteral("INSERT INTO autofill_encrypted (password_encrypted, server) VALUES (?,?)"));
        }
        ...
        query.prepare(QStringLiteral("DELETE FROM autofill_encrypted WHERE server = ?"));
        query.addBindValue(INTERNAL_SERVER_ID);
        query.exec();
    ...
    if (m_backend->isActive() && !m_backend->isMasterPasswordSetted()) {
        ...
        QMessageBox::information(this, AutoFill::tr("Warning!"),
                                 AutoFill::tr("This backend needs a master password to be set! "
                                              "Midori Browser just switches to its default backend"));
        // active default backend
        mApp->autoFill()->passwordManager()->switchBackend(QStringLiteral("database"));
        return;
    }
}
...
void MasterPasswordDialog::clearMasterPasswordAndConvert(bool forcedAskPass)
{
    ...
    if (m_backend->hasPermission()) {
        ...
        PasswordBackend* databaseBackend = mApp->autoFill()->passwordManager()->availableBackends().value(QStringLiteral("database"));
        if (!databaseBackend) {
            return;
        }
