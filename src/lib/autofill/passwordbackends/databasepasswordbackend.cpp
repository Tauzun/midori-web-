...
QVector<PasswordEntry> DatabasePasswordBackend::getEntries(const QUrl &url)
{
    ...
    query.prepare(QStringLiteral("SELECT id, username, password, data FROM autofill "
...
QVector<PasswordEntry> DatabasePasswordBackend::getAllEntries()
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id, server, username, password, data FROM autofill"));
    query.exec();
...
void DatabasePasswordBackend::addEntry(const PasswordEntry &entry)
{
    ...
    if (entry.data.isEmpty()) {
        ...
        QSqlQuery query(SqlDatabase::instance()->database());
        query.prepare(QStringLiteral("SELECT username FROM autofill WHERE server=?"));
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("INSERT INTO autofill (server, data, username, password, last_used) "
...
bool DatabasePasswordBackend::updateEntry(const PasswordEntry &entry)
{
    ...
    // Data is empty only for HTTP/FTP authorization
    if (entry.data.isEmpty()) {
        query.prepare(QStringLiteral("UPDATE autofill SET username=?, password=? WHERE server=?"));
        query.bindValue(0, entry.username);
        query.bindValue(1, entry.password);
        query.bindValue(2, entry.host);
    }
    else {
        query.prepare(QStringLiteral("UPDATE autofill SET data=?, username=?, password=? WHERE id=?"));
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("UPDATE autofill SET last_used=strftime('%s', 'now') WHERE id=?"));
    query.addBindValue(entry.id);
    query.exec();
}
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("DELETE FROM autofill WHERE id=?"));
    query.addBindValue(entry.id);
    query.exec();
}
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.exec(QStringLiteral("DELETE FROM autofill"));
}
