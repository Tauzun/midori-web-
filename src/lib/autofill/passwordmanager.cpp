...
{
    m_backends[QStringLiteral("database")] = m_databaseBackend;
    m_backends[QStringLiteral("database-encrypted")] = m_databaseEncryptedBackend;
}
...
void PasswordManager::loadSettings()
{
    ...
    QString backendId = settings.value(QStringLiteral("Backend"), QStringLiteral("database")).toString();
    settings.endGroup();

    if (m_backend) {
        m_backend->setActive(false);
    }
    m_backend = m_backends[m_backends.contains(backendId) ? backendId : QStringLiteral("database")];
