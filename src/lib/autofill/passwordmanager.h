...
struct MIDORI_EXPORT PasswordEntry {
    QVariant id;
    QString host;
    QString username;
    QString password;
    ...
    friend MIDORI_EXPORT QDataStream &operator<<(QDataStream &stream, const PasswordEntry &entry);
    friend MIDORI_EXPORT QDataStream &operator>>(QDataStream &stream, PasswordEntry &entry);
};

class MIDORI_EXPORT PasswordManager : public QObject
{
    Q_OBJECT
public:
