    ...
    if (m_useExternalManager) {
        ...
        case Save:
            ...
            if (!downloadPath.isEmpty()) {
                ...
                Settings().setValue(QStringLiteral("DownloadManager/lastDownloadPath"), m_lastDownloadPath);
        ...
        downloadPath = QzTools::ensureUniqueFilename(m_downloadPath + QLatin1Char('/') + fileName);
    }
...
void DownloadManager::downloadFinished(bool success)
{
    ...
    if (downloadingAllFilesFinished) {
        if (success && qApp->activeWindow() != this) {
            mApp->desktopNotifications()->showNotification(QIcon(QStringLiteral(":icons/other/download.svg")).pixmap(48), tr("Download Finished"), tr("All files have been successfully downloaded."));
            if (!m_closeOnFinish) {
                raise();
                activateWindow();
            }
