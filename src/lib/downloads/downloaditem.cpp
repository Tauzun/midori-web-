    ...
    , m_total(downloadItem->totalBytes())
{
    ...
    ui->cancelButton->setPixmap(QIcon(QStringLiteral(":icons/menu/process-stop.svg")).pixmap(20, 20));
    ui->pauseResumeButton->setPixmap(QIcon(QStringLiteral(":icons/menu/process-pause.svg")).pixmap(20, 20));
...
void DownloadItem::pauseResume()
{
    if (m_download->isPaused()) {
        ...
        m_download->pause();
        ui->pauseResumeButton->setPixmap(QIcon(QStringLiteral(":icons/menu/process-resume.svg")).pixmap(20, 20));
...
{
    QMenu menu;
    menu.addAction(QIcon(QStringLiteral(":icons/menu/document-open.svg")), tr("Open File"), this, &DownloadItem::openFile);

    menu.addAction(tr("Open Folder"), this, &DownloadItem::openFolder);
    menu.addSeparator();
    menu.addAction(QIcon(QStringLiteral(":icons/menu/edit-copy.svg")), tr("Copy Download Link"), this, &DownloadItem::copyDownloadLink);
    menu.addSeparator();
    menu.addAction(QIcon(QStringLiteral(":icons/menu/process-stop.svg")), tr("Cancel downloading"), this, &DownloadItem::stop)->setEnabled(m_downloading);

    if (m_download->isPaused()) {
        menu.addAction(QIcon(QStringLiteral(":icons/menu/process-resume.svg")), tr("Resume downloading"), this, &DownloadItem::pauseResume)->setEnabled(m_downloading);
    } else {
        menu.addAction(QIcon(QStringLiteral(":icons/menu/process-pause.svg")), tr("Pause downloading"), this, &DownloadItem::pauseResume)->setEnabled(m_downloading);
    }

    menu.addAction(QIcon(QStringLiteral(":icons/menu/list-remove.svg")), tr("Remove From List"), this, &DownloadItem::clear)->setEnabled(!m_downloading);
...
{
#ifdef Q_OS_WIN
    QString winFileName = QStringLiteral("%1/%2").arg(m_path, m_fileName);

    if (m_downloading) {
        winFileName.append(QStringLiteral(".download"));
    }
