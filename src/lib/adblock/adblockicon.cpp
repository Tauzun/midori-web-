...
{
    setTitle(tr("AdBlock"));
    setIcon(QIcon(QStringLiteral(":adblock/data/adblock.png")));
...
{
    return QStringLiteral("adblock-icon");
}
...
void AdBlockIcon::clicked(ClickController *controller)
{
    ...
    if (!pageUrl.host().isEmpty() && manager->isEnabled() && manager->canRunOnScheme(pageUrl.scheme())) {
        ...
        const QString hostFilter = QStringLiteral("@@||%1^$document").arg(host);
        const QString pageFilter = QStringLiteral("@@|%1|$document").arg(pageUrl.toString());
