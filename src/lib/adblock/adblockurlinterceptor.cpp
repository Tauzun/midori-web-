...
void AdBlockUrlInterceptor::interceptRequest(QWebEngineUrlRequestInfo &request)
{
    ...
    if (request.resourceType() == QWebEngineUrlRequestInfo::ResourceTypeMainFrame) {
        QString page;
        page.append(QzTools::readAllFileContents(QStringLiteral(":adblock/data/adblock.html")));
        page.replace(QStringLiteral("%FAVICON%"), QStringLiteral("qrc:adblock/data/adblock_big.png"));
        page.replace(QStringLiteral("%IMAGE%"), QStringLiteral("qrc:adblock/data/adblock_big.png"));
        page.replace(QStringLiteral("%TITLE%"), tr("Blocked content"));
        page.replace(QStringLiteral("%RULE%"), tr("Blocked by <i>%1 (%2)</i>").arg(ruleFilter, ruleSubscription));
        page = QzTools::applyDirectionToPage(page);
        request.redirect(QUrl(QString::fromUtf8(QByteArray("data:text/html;charset=utf-8;base64,") + page.toUtf8().toBase64())));
    } else {
        request.block(true);
    }
