...
static const bool kEnableJsOutput = qEnvironmentVariableIsSet("BHAWK_ENABLE_JS_OUTPUT");
static const bool kEnableJsNonBlockDialogs = qEnvironmentVariableIsSet("BHAWK_ENABLE_JS_NONBLOCK_DIALOGS");
    ...
    , m_secureStatus(false)
{
    ...
    setBackgroundColor(MainApplication::palette().color(QPalette::Window)); // Prevents white flashes between page loads and transitions
    ExternalJsObject::setupWebChannel(channel, this);
    setWebChannel(channel, jsAppWorldSafe);

    connect(this, &QWebEnginePage::loadStarted, this, &WebPage::started);
...
{
    runJavaScript(QStringLiteral("window.scrollTo(window.scrollX + %1, window.scrollY + %2)").arg(x).arg(y), jsAppWorldSafe);
}

void WebPage::setScrollPosition(const QPointF &pos)
{
    const QPointF v = mapToViewport(pos.toPoint());
    runJavaScript(QStringLiteral("window.scrollTo(%1, %2)").arg(v.x()).arg(v.y()), jsAppWorldSafe);
}
...
bool WebPage::started()
{
    // Add operations here to occur when page loading begins
    return true;
...
{
    // Reset the page (webview widget) background color after it is changed above to prevent white flash
...
void WebPage::handleUnknownProtocol(const QUrl &url)
{
    ...
    const QString text = tr("Midori Browser cannot handle <b>%1:</b> links. The requested link "
                            "is <ul><li>%2</li></ul>Do you want Midori Browser to try "
...
bool WebPage::acceptNavigationRequest(const QUrl &url, QWebEnginePage::NavigationType type, bool isMainFrame)
{
    ...
    if (url.scheme() == QLatin1String("browser")) {
        if (url.path() == QLatin1String("AddSearchProvider")) {
            QUrlQuery query(url);
            mApp->searchEnginesManager()->addEngine(QUrl(query.queryItemValue(QStringLiteral("url"))));
            return false;
        }
    }
    #if defined(ENABLE_KDE_FRAMEWORKS_INTEGRATION_PLUGIN) // Temporary hack to prevent building on Windows
    ...
    if (result) {
        if (isMainFrame) {
            const bool isWeb = url.scheme() == QLatin1String("http") || url.scheme() == QLatin1String("https") || url.scheme() == QLatin1String("file");
