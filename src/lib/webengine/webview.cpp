...
void WebView::load(const LoadRequest &request) {
    ...
    if ( isUrlValid(reqUrl) ) {
        ...
        if (qzSettings->httpsByDefault) {
            ...
            if ( reqUrl.scheme() == QLatin1String("http") ) {

                QUrl newScheme(reqUrl);
                newScheme.setScheme("https");
                const QUrl rqUrl(newScheme);
                loadRequest(rqUrl);
...
void WebView::printPage()
{
    ...
    printer->setCreator(tr("Midori Browser %1 (%2)").arg(Qz::VERSION, Qz::WWWADDRESS));
...
void WebView::copyPlainTextToClipboard()
{
    if (QAction* action = qobject_cast<QAction*>(sender())) {
...
{
    page()->runJavaScript(QStringLiteral("document.contentType"), WebPage::jsAppWorldSafe, [this](const QVariant &res) {
...
void WebView::showSource()
{
    ...
    if (url().scheme() == QLatin1String("view-source") || url().scheme() == QLatin1String("browser") || url().scheme() == QLatin1String("qrc")) {
...
void WebView::createPageContextMenu(QMenu* menu)
{
    ...
    menu->addSeparator();
    menu->addAction(QIcon(QStringLiteral(":icons/menu/bookmark-new.svg")), tr("Book&mark page"), this, &WebView::bookmarkLink);
    menu->addAction(QIcon(QStringLiteral(":icons/menu/document-save.svg")), tr("&Save page as..."), this, &WebView::savePageAs);
    menu->addAction(QIcon(QStringLiteral(":icons/menu/edit-copy.svg")), tr("&Copy page link"), this, &WebView::copyLinkToClipboard)->setData(url());
    menu->addAction(QIcon(QStringLiteral(":icons/menu/mail-message-new.svg")), tr("Send page link..."), this, &WebView::sendPageByMail);
    menu->addSeparator();
...
void WebView::createSelectedTextContextMenu(QMenu* menu, const WebHitTestResult &hitTest)
{
    ...
    if (isUrlValid(guessedUrl)) {
        Action* act = new Action(QIcon(QStringLiteral(":icons/menu/document-open-remote.svg")), tr("Go to &web address"));
...
void WebView::createMediaContextMenu(QMenu *menu, const WebHitTestResult &hitTest)
{
    ...
    menu->addSeparator();
    menu->addAction(paused ? tr("&Play") : tr("&Pause"), this, &WebView::toggleMediaPause)->setIcon(QIcon(paused ? QStringLiteral(":icons/menu/process-resume.svg") : QStringLiteral(":icons/menu/process-pause.svg")));
    menu->addAction(muted ? tr("Un&mute") : tr("&Mute"), this, &WebView::toggleMediaMute)->setIcon(QIcon(muted ? QStringLiteral(":icons/menu/tab-mute.svg") : QStringLiteral(":icons/menu/tab-unmute.svg")));
    menu->addSeparator();
    menu->addAction(QIcon(QStringLiteral(":icons/menu/edit-copy.svg")), tr("&Copy Media Address"), this, &WebView::copyLinkToClipboard)->setData(hitTest.mediaUrl());
    menu->addAction(QIcon(QStringLiteral(":icons/menu/mail-message-new.svg")), tr("&Send Media Address"), this, &WebView::sendTextByMail)->setData(hitTest.mediaUrl().toEncoded());
    menu->addAction(QIcon(QStringLiteral(":icons/menu/document-save.svg")), tr("Save Media To &Disk"), this, &WebView::downloadMediaToDisk);
}
...
void WebView::checkForForm(QAction *action, const QPoint &pos)
{
    ...
    page()->runJavaScript(Scripts::getFormData(m_clickedPos), WebPage::jsAppWorldSafe, [this, act](const QVariant &res) {
        const QVariantMap &map = res.toMap();
        if (!act || map.isEmpty())
            return;

        const QUrl url = map.value(QStringLiteral("action")).toUrl();
        ...
        if (!url.isEmpty() && (method == QLatin1String("get") || method == QLatin1String("post"))) {
            act->setVisible(true);
            act->setIcon(QIcon(QStringLiteral(":icons/menu/search-icon.svg")));
...
{
    page()->runJavaScript(Scripts::getFormData(m_clickedPos), WebPage::jsAppWorldSafe, [this](const QVariant &res) {
...
{
    page()->runJavaScript("a.addSpeedDial()", WebPage::jsAppWorldSafe);
}

void WebView::configureSpeedDial()
{
    page()->runJavaScript("a.configureSpeedDial()", WebPage::jsAppWorldSafe);
}

void WebView::reloadAllSpeedDials()
{
    page()->runJavaScript("a.reloadAll()", WebPage::jsAppWorldSafe);
}
...
void WebView::initializeActions()
{
    ...
    undoAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-undo.svg")));
    ...
    redoAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-redo.svg")));

    QAction* cutAction = pageAction(QWebEnginePage::Cut);
    cutAction->setText(tr("&Cut"));
    cutAction->setShortcut(QKeySequence("Ctrl+X"));
    cutAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    cutAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-cut.svg")));

    QAction* copyAction = pageAction(QWebEnginePage::Copy);
    copyAction->setText(tr("&Copy"));
    copyAction->setShortcut(QKeySequence("Ctrl+C"));
    copyAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    copyAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-copy.svg")));

    QAction* pasteAction = pageAction(QWebEnginePage::Paste);
    pasteAction->setText(tr("&Paste"));
    pasteAction->setShortcut(QKeySequence("Ctrl+V"));
    pasteAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    pasteAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-paste.svg")));

    QAction* selectAllAction = pageAction(QWebEnginePage::SelectAll);
    selectAllAction->setText(tr("Select All"));
    selectAllAction->setShortcut(QKeySequence("Ctrl+A"));
    selectAllAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    selectAllAction->setIcon(QIcon(QStringLiteral(":icons/menu/edit-select-all.svg")));

    QAction* reloadAction = pageAction(QWebEnginePage::Reload);
    reloadAction->setText(tr("&Reload"));
    reloadAction->setIcon(QIcon(QStringLiteral(":icons/menu/view-refresh.svg")));

    QAction* stopAction = pageAction(QWebEnginePage::Stop);
    stopAction->setText(tr("S&top"));
    stopAction->setIcon(QIcon(QStringLiteral(":icons/menu/process-stop.svg")));
