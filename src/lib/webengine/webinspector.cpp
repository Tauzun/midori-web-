    ...
    , m_view(Q_NULLPTR)
{
    ...
    setObjectName(QStringLiteral("web-inspector"));
    setMinimumHeight(80);

    m_height = Settings().value(QStringLiteral("Web-Inspector/height"), 240).toInt();
    m_windowSize = Settings().value(QStringLiteral("Web-Inspector/windowSize"), QSize(640, 480)).toSize();
...
WebInspector::~WebInspector()
{
    ...
    if (isWindow()) {
        Settings().setValue(QStringLiteral("Web-Inspector/windowSize"), size());
    } else {
        Settings().setValue(QStringLiteral("Web-Inspector/height"), height());
    }
}
...
void WebInspector::loadFinished()
{
    ...
    if (!isWindow()) {
        page()->runJavaScript(QLatin1String("var button = Components.dockController._closeButton;"
