...
private:
    void paintEvent(QPaintEvent *ev) override
    {
        ...
        QPainter p(this);
        if (mApp->styleName() == QLatin1String("breeze")) {
...
WebScrollBarManager::WebScrollBarManager(QObject *parent)
    : QObject(parent)
{
    m_scrollbarJs = QLatin1String("(function() {"
...
{
    m_enabled = Settings().value(QStringLiteral("Web-Browser-Settings/UseNativeScrollbars"), true).toBool(); // Does nothing on Windows 10
...
void WebScrollBarManager::addWebView(WebView *view)
{
    ...
    connect(view->page(), &WebPage::contentsSizeChanged, data->vscrollbar, [=]() {
        const QString source = QLatin1String("var out = {"
        ...
        view->page()->runJavaScript(source, WebPage::jsAppWorldSafe, [=](const QVariant &res) {
            if (!p || !m_scrollbars.contains(view)) {
                return;
            }
            const QVariantMap map = res.toMap();
            data->vscrollbarVisible = map.value(QStringLiteral("vertical")).toBool();
            data->hscrollbarVisible = map.value(QStringLiteral("horizontal")).toBool();
            updateValues();
        });
    });
...
void WebScrollBarManager::createUserScript(int thickness)
{
    ...
    script.setName(QStringLiteral("_browser_scrollbar"));
...
{
    QWebEngineScript script = mApp->webProfile()->scripts()->findScript(QStringLiteral("_browser_scrollbar"));
