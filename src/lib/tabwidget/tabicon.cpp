...
{
    setObjectName(QStringLiteral("tab-icon"));
...
void TabIcon::updateIcon()
{
    ...
    if (m_sitePixmap.isNull()) {
        if (m_tab->url().isEmpty() || m_tab->url().scheme() == QLatin1String("browser")) {
            hide();
        } else {
            m_hideTimer->start();
        }
...
TabIcon::Data *TabIcon::data()
{
    ...
    if (!data) {
        ...
        data->animationPixmap = QIcon(QStringLiteral(":icons/other/loading.png")).pixmap(288, 16);
        data->framesCount = data->animationPixmap.width() / data->animationPixmap.height();
        data->audioPlayingPixmap = QIcon(QStringLiteral(":icons/other/audioplaying.svg")).pixmap(16);
        data->audioMutedPixmap = QIcon(QStringLiteral(":icons/other/audiomuted.svg")).pixmap(16);
    }
    return data;
}
