    ...
    , m_window(window)
{
    ...
    ui->titleLabel->setText(tr("<b>Site %1<b/>").arg(host));

    if (scheme == QLatin1String("https")) {

        if ( mApp->networkManager()->rejectedSslErrors(view->url().host()) ) {
            ui->secureLabel->setText(tr("Your connection to this site is <b>unsecured</b>."));
            ui->secureIcon->setPixmap(QPixmap(":/icons/menu/document-not-encrypted.svg"));
        } else if ( mApp->networkManager()->ignoredSslErrors(view->url().host()) ) {
            ui->secureLabel->setText(tr("Your connection to this site is <b>unsecured</b>."));
            ui->secureIcon->setPixmap(QPixmap(":/icons/menu/document-not-encrypted.svg"));
        } else {
            ui->secureLabel->setText(tr("Your connection to this site is <b>secured</b>."));
            ui->secureIcon->setPixmap(QPixmap(":/icons/menu/document-encrypted.svg"));
        }
    ...
    connect(ui->pushButton, &QAbstractButton::clicked, m_window->action(QStringLiteral("Tools/SiteInfo")), &QAction::trigger);
