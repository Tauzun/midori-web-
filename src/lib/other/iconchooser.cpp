...
void IconChooser::searchIcon(const QString &string)
{
    ...
    query.prepare(QStringLiteral("SELECT icon FROM icons WHERE url GLOB ? LIMIT 20"));
    query.addBindValue(QString(QLatin1String("*%1*")).arg(QzTools::escapeSqlGlobString(string)));
    query.exec();
