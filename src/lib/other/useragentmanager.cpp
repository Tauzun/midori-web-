    ...
    , m_usePerDomainUserAgent(false)
{
    ...
    // m_defaultUserAgent.replace(QRegularExpression(QStringLiteral("QtWebEngine/[^\\s]+")), QStringLiteral("BlueHawk/%1").arg(Qz::VERSION));
    m_defaultUserAgent.replace(QRegularExpression(QStringLiteral("QtWebEngine/[^\\s]+")), QStringLiteral(""));
    m_defaultUserAgent.replace(QStringLiteral("NT 6.2"), QStringLiteral("NT 10.0"));
}
