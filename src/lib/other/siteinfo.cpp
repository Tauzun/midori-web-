    ...
    , m_baseUrl(view->url())
{
    ...
    ui->listWidget->item(0)->setIcon(QIcon(QStringLiteral(":icons/menu/document-properties.svg")));
    ui->listWidget->item(1)->setIcon(QIcon(QStringLiteral(":icons/menu/applications-graphics.svg")));
    ...
    if (m_view->url().scheme() == QLatin1String("https")) {
        if ( mApp->networkManager()->rejectedSslErrors(m_view->url().host()) ) {
            ui->securityLabel->setText(tr("<b>Connection Not Encrypted.</b>"));
        } else if ( mApp->networkManager()->ignoredSslErrors(m_view->url().host()) ) {
            ui->securityLabel->setText(tr("<b>Connection Not Encrypted.</b>"));
        } else {
    ...
    m_view->page()->runJavaScript(QStringLiteral("document.charset"), WebPage::jsAppWorldSafe, [this](const QVariant &res) {
        ui->encodingLabel->setText(res.toString());
    });
    // Meta
    m_view->page()->runJavaScript(Scripts::getAllMetaAttributes(), WebPage::jsAppWorldSafe, [this](const QVariant &res) {
    ...
    // Images
    m_view->page()->runJavaScript(Scripts::getAllImages(), WebPage::jsAppWorldSafe, [this](const QVariant &res) {
        const QVariantList &list = res.toList();
        for (const QVariant &val : list) {
...
bool SiteInfo::canShowSiteInfo(const QUrl &url)
{
    ...
    if (url.scheme() == QLatin1String("browser") || url.scheme() == QLatin1String("view-source") || url.scheme() == QLatin1String("plug-in"))
        return false;
...
void SiteInfo::imagesCustomContextMenuRequested(const QPoint &p)
{
    ...
    QMenu menu;
    menu.addAction(QIcon(QStringLiteral(":icons/menu/edit-copy.svg")), tr("Copy Image Location"), this, SLOT(copyActionData()))->setData(item->text(1));
    menu.addAction(tr("Copy Image Name"), this, SLOT(copyActionData()))->setData(item->text(0));
    menu.addSeparator();
    menu.addAction(QIcon(QStringLiteral(":icons/menu/document-save.svg")), tr("Save Image to Disk"), this, SLOT(saveImage()));
