...
void ProtocolHandlerManager::registerHandler(const QString &scheme, const QUrl &url)
{
    ...
    urlString.replace(QLatin1String("%25s"), QLatin1String("%s"));
    ...
    page->setHtml(QStringLiteral("<script>navigator.registerProtocolHandler('%1', '%2', '')</script>").arg(scheme, urlString), url);
}
