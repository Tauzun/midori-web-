...
void HistoryMenu::aboutToShow()
{
    ...
    query.exec(QStringLiteral("SELECT title, url FROM history ORDER BY date DESC LIMIT 10"));
...
void HistoryMenu::aboutToShowClosedTabs()
{
    ...
    if (m_menuClosedTabs->isEmpty()) {
        ...
        m_menuClosedTabs->addAction(QIcon(QStringLiteral(":icons/menu/tab-restore.svg")), tr("Restore All Closed Tabs"), tabWidget, &TabWidget::restoreAllClosedTabs);
        m_menuClosedTabs->addAction(QIcon(QStringLiteral(":icons/menu/edit-clear.svg")), tr("Clear list"), tabWidget, &TabWidget::clearClosedTabsList);
    }
}
...
void HistoryMenu::aboutToShowClosedWindows()
{
    ...
    if (m_menuClosedWindows->isEmpty()) {
        ...
        m_menuClosedWindows->addAction(QIcon(QStringLiteral(":icons/menu/process-restore.svg")), tr("Restore All Closed Windows"), manager, &ClosedWindowsManager::restoreAllClosedWindows);
        m_menuClosedWindows->addAction(QIcon(QStringLiteral(":icons/menu/edit-clear.svg")), tr("Clear list"), manager, &ClosedWindowsManager::clearClosedWindows);
    }
}
...
void HistoryMenu::init()
{
    ...
    act = addAction(QIcon(QStringLiteral(":icons/menu/go-home.svg")), tr("&Home"), this, &HistoryMenu::goHome);
    act->setShortcut(QKeySequence(Qt::ALT + Qt::Key_Home));

    act = addAction(QIcon(QStringLiteral(":icons/menu/history.svg")), tr("Show &All History"), this, &HistoryMenu::showHistoryManager);
