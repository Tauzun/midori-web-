...
void History::addHistoryEntry(const QUrl &url, QString title)
{
    ...
    const QStringList schemes = {
        QStringLiteral("http"), QStringLiteral("https"), QStringLiteral("ftp"), QStringLiteral("file")
    };
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT id FROM history WHERE url=?"));
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.exec(QStringLiteral("DELETE FROM history"));
...
QList<HistoryEntry> History::searchHistoryEntry(const QString &text)
{
    ...
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT count, date, id, title, url FROM history WHERE title LIKE ? OR url LIKE ?"));
...
{
    QSqlQuery query(SqlDatabase::instance()->database());
    query.prepare(QStringLiteral("SELECT count, date, id, title, url FROM history WHERE url = ?"));
