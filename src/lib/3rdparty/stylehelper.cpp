...
void StyleHelper::verticalGradient(QPainter* painter, const QRect &spanRect, const QRect &clipRect, bool lightColored)
{
    if (StyleHelper::usePixmapCache()) {
        ...
        QPixmap pixmap;
        const bool findPmapCache = QPixmapCache::find(key, &pixmap);
        if (!findPmapCache) {
...
{
    QPixmap cache;
    QString pixmapName = QStringLiteral("icon %0 %1 %2").arg(icon.cacheKey()).arg(iconMode).arg(rect.height());
    const bool findPmapCache = QPixmapCache::find(pixmapName, &cache);
