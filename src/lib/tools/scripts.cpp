...
QString Scripts::setupWebChannel()
{
    QString source =  QLatin1String("(function() {"
                           ...
                           "        event.initEvent('_browser_external_created', true, true);"
                           "        window._browser_external = true;"
                           "        document.dispatchEvent(event);"
                           "    }"
                           "}"
                           ""
                           "if (self !== top) {"
                           "    if (top._browser_external)"
                           "        registerExternal(top.external);"
                           "    else"
                           "        top.document.addEventListener('_browser_external_created', function() {"
                           "            registerExternal(top.external);"
                           "        });"
                           "    return;"
                           "}"
    ...
    return source.arg(QzTools::readAllFileContents(QStringLiteral(":/qtwebchannel/qwebchannel.js")));
}
...
{
    QString source = QzTools::readAllFileContents(QStringLiteral(":html/speeddial.user.js"));
    QString vanta = QStringLiteral("VANTA.WAVES({"
                    ...
                    "zoom: 0.88"
                    "})");
    source.replace(QLatin1String("%JQUERY%"), QzTools::readAllFileContents(QStringLiteral(":html/jquery.js")));
    source.replace(QLatin1String("%JQUERY-UI%"), QzTools::readAllFileContents(QStringLiteral(":html/jquery-ui.js")));
    source.replace(QLatin1String("%THREEJS%"), QzTools::readAllFileContents(QStringLiteral(":html/vantajs/three.min.js")));
    source.replace(QLatin1String("%VANTA-WAVES%"), QzTools::readAllFileContents(QStringLiteral(":html/vantajs/vanta.waves.min.js")));
    source.replace(QLatin1String("%VANTA-CODE%"), vanta);
...
QString Scripts::completeFormData(const QByteArray &data)
{
    ...
    QString d = data;
    d.replace(QLatin1String("'"), QLatin1String("\\'"));
    return source.arg(d);
}
