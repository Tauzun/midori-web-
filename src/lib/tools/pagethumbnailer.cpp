    ...
    , m_loadTitle(false)
{
    ...
    m_view->load(QUrl(":/html/broken-page.svg"));
    m_view->show();
}
...
void PageThumbnailer::start()
{
    ...
    if (WebView::isUrlValid(m_url)) {
        m_view->load(QUrl(m_url));
        QTimer::singleShot(3000, this, [this]() {
            emit thumbnailCreated(m_view->grab());
        });
    } else {
...
{
    return Scripts::setCss(QStringLiteral("::-webkit-scrollbar{display:none}"));
}
