...
QUrl QzTools::pixmapToDataUrl(const QPixmap &pix)
{
    ...
    return data.isEmpty() ? QUrl() : QUrl(QStringLiteral("data:image/png;charset=utf-8;base64,") + data);
}
...
bool QzTools::copyRecursively(const QString &sourcePath, const QString &targetPath)
{
    ...
    if (srcFileInfo.isDir() && !srcFileInfo.isSymLink()) {
        ...
        for (const QString &fileName : fileNames) {
            const QString newSourcePath = sourcePath + QLatin1Char('/') + fileName;
            const QString newTargetPath = targetPath + QLatin1Char('/') + fileName;
...
{
    if (!str.startsWith(QLatin1String("xn--")))
        return str;
...
void QzTools::setWmClass(const QString &name, const QWidget* widget)
{
    ...
    const QByteArray classData = mApp->wmClass().isEmpty() ? QByteArrayLiteral("Midori") : mApp->wmClass();
...
QString QzTools::operatingSystemLong()
{
    ...
    return QzTools::operatingSystem() + QStringLiteral(" ") + arch;
}
