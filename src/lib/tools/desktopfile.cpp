...
DesktopFile::DesktopFile(const QString &fileName)
{
    ...
    m_settings->beginGroup(QStringLiteral("Desktop Entry"));
}
...
{
    return value(QStringLiteral("Name"), true).toString();
}

QString DesktopFile::comment() const
{
    return value(QStringLiteral("Comment"), true).toString();
}

QString DesktopFile::type() const
{
    return value(QStringLiteral("Type")).toString();
}
...
QVariant DesktopFile::value(const QString &key, bool localized) const
{
    ...
    if (localized) {
        ...
        QString localeKey = QStringLiteral("%1[%2]").arg(key, locale.name());
        if (m_settings->contains(localeKey)) {
            return m_settings->value(localeKey);
        }
        localeKey = QStringLiteral("%1[%2]").arg(key, locale.bcp47Name());
        if (m_settings->contains(localeKey)) {
            return m_settings->value(localeKey);
        }
        const int i = locale.name().indexOf(QLatin1Char('_'));
        if (i > 0) {
            localeKey = QStringLiteral("%1[%2]").arg(key, locale.name().left(i));
            if (m_settings->contains(localeKey)) {
                return m_settings->value(localeKey);
            }
        }
...
bool DesktopFile::tryExec() const
{
    ...
    const QString exec = m_settings->value(QStringLiteral("TryExec")).toString();
