...
QJsonArray RecoveryJsObject::restoreData() const
{
    ...
    for (const BrowserWindow::SavedWindow &w : windows) {
        ...
        for (const WebTab::SavedTab &t : windowTabs) {
            ...
            QJsonObject tab;
            tab[QStringLiteral("tab")] = j;
            tab[QStringLiteral("icon")] = QzTools::pixmapToDataUrl(icon.pixmap(16)).toString();
            tab[QStringLiteral("title")] = t.title;
            tab[QStringLiteral("url")] = t.url.toString();
            tab[QStringLiteral("pinned")] = t.isPinned;
            tab[QStringLiteral("current")] = w.currentTab == j;
            tabs.append(tab);
            j++;
        }

        QJsonObject window;
        window[QStringLiteral("window")] = i++;
        window[QStringLiteral("tabs")] = tabs;
        out.append(window);
    }
