...
SessionManager::SessionManager(QObject* parent)
    : QObject(parent)
    , m_firstBackupSession(DataPaths::currentProfilePath() + QLatin1String("/session.dat.old"))
    , m_secondBackupSession(DataPaths::currentProfilePath() + QLatin1String("/session.dat.old1"))
{
...
void SessionManager::fillSessionsMetaDataListIfNeeded()
{
    ...
    const QFileInfoList sessionFiles = QFileInfoList() << QFileInfo(defaultSessionPath()) << dir.entryInfoList({QStringLiteral("*.*")}, QDir::Files, QDir::Time);
...
{
    return DataPaths::currentProfilePath() + QLatin1String("/session.dat");
}
