...
void LocationCompleterDelegate::paint(QPainter* painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    const int newPointSize = linkFont.pointSize() - 1;

    if (newPointSize > 1) {
        linkFont.setPointSize(newPointSize);
    } else {
        linkFont.setPointSize(10);
    }
    ...
    if (isSearchSuggestion || (isVisitSearchItem && isWebSearch)) {
        pixmap = QIcon(QStringLiteral(":icons/menu/search-icon.svg")).pixmap(iconSize, iconMode);
    }
    ...
    const int maxChars = (opt.rect.width() - leftPosition) / opt.fontMetrics.horizontalAdvance(QLatin1Char('i'));
    QString link;
    ...
    if (qzSettings->showSwitchTab && !m_forceVisitItem && tabPos != -1) {
        const QIcon tabIcon = QIcon(QStringLiteral(":icons/menu/tab.svg"));
