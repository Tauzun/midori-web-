    ...
    , m_progressVisible(false)
{
    ...
    editAction(PasteAndGo)->setIcon(QIcon(QStringLiteral(":icons/menu/edit-paste-go.svg")));
...
LocationBar::LoadAction LocationBar::loadAction(const QString &text)
{
    ...
    if (guessedUrl.isValid()) {
        ...
        const bool forceLoad = guessedUrl.scheme() == QLatin1String("javascript");
...
void LocationBar::textEdited(const QString &text)
{
    ...
    if (!text.isEmpty()) {
        ...
        m_siteIcon->setIcon(QIcon(QStringLiteral(":icons/menu/search-icon.svg")));
    }
    else {
...
void LocationBar::updateSiteIcon() {
    ...
    if (m_webView) {
        ...
        const QString urlScheme = m_webView->url().scheme();

        if (urlScheme == "https") {

            if ( mApp->networkManager()->rejectedSslErrors(m_webView->url().host()) ) {

                icon = QIcon(QStringLiteral(":icons/menu/document-not-encrypted.svg"));

            } else if ( mApp->networkManager()->ignoredSslErrors(m_webView->url().host()) ) {
                ...
                if (property("secured").toBool()) {
                    ...
                    icon = QIcon(QStringLiteral(":icons/menu/document-encrypted.svg"));
            ...
            icon = QIcon(QStringLiteral(":icons/other/webpage.svg"));

        } else if (urlScheme == "ftp") {

            icon = QIcon(QStringLiteral(":icons/other/webpage.svg"));

        } else {

            icon = QIcon(QStringLiteral(":icons/menu/document-not-encrypted-2.svg"));
    ...
    if (m_completer->isVisible()) {
        ...
        icon = QIcon(QStringLiteral(":icons/menu/search-icon.svg"));
