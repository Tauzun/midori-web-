    ...
    , m_window(window)
{
    ...
    m_buttonTools = new ToolButton(this);
    m_buttonTools->setObjectName("navigation-button-tools");
    m_buttonTools->setPopupMode(QToolButton::InstantPopup);
    m_buttonTools->setToolbarButtonLook(true);
    m_buttonTools->setToolTip(tr("Tools"));
    ...
    connect(m_buttonTools, &ToolButton::aboutToShowMenu, this, &NavigationBar::aboutToShowToolsMenu);
    ...
    connect(m_buttonHome, &QAbstractButton::clicked, m_window, &BrowserWindow::goHome);
    connect(m_buttonHome, &ToolButton::middleMouseClicked, m_window, &BrowserWindow::goHomeInNewTab);
    connect(m_buttonHome, &ToolButton::controlClicked, m_window, &BrowserWindow::goHomeInNewTab);
    connect(m_buttonAddTab, &QAbstractButton::clicked, m_window, &BrowserWindow::addTab);
    connect(m_buttonAddTab, &ToolButton::middleMouseClicked, m_window->tabWidget(), &TabWidget::addTabFromClipboard);
    connect(m_exitFullscreen, &QAbstractButton::clicked, m_window, &BrowserWindow::toggleFullScreen);

    addWidget(backNextWidget, QStringLiteral("button-backforward"), tr("Back and Forward buttons"));
...
void NavigationBar::aboutToShowHistoryBackMenu()
{
    ...
    m_menuBack->addAction(QIcon(QStringLiteral(":icons/menu/edit-clear.svg")), tr("Clear history"), this, &NavigationBar::clearHistory);
}
...
void NavigationBar::aboutToShowHistoryNextMenu()
{
    ...
    m_menuForward->addAction(QIcon(QStringLiteral(":icons/menu/edit-clear.svg")), tr("Clear history"), this, &NavigationBar::clearHistory);
}
    ...
    for (const WidgetData &data : qAsConst(m_widgets)) {
        ...
        if (button && (!button->isVisible() || !m_layoutIds.contains(data.id))) {
            ...
            if (!button->badgeText().isEmpty()) {
                title.append(QStringLiteral(" (%1)").arg(button->badgeText()));
            }
...
void NavigationBar::toolActionActivated()
{
    ...
    if (!data.button) {
        ...
        m_buttonTools = qobject_cast<ToolButton*>(m_widgets.value(QStringLiteral("button-tools")).widget);
    if (!m_buttonTools) {
        return;
    }

    AbstractButtonInterface::ClickController *c = new AbstractButtonInterface::ClickController;
    c->visualParent = m_buttonTools;
    c->popupPosition = [=](const QSize &size) {
        QPoint pos = m_buttonTools->mapToGlobal(m_buttonTools->rect().bottomRight());
        if (QApplication::isRightToLeft()) {
            pos.setX(pos.x() - m_buttonTools->rect().width());
        } else {
    ...
    c->popupClosed = [=]() {
        m_buttonTools->setDown(false);
        delete c;
    };
    emit data.button->clicked(c);
    if (c->popupOpened) {
        m_buttonTools->setDown(true);
    } else {
        c->popupClosed();
    }
}
...
void NavigationBar::loadSettings()
{
    const QStringList defaultIds = {
        QStringLiteral("button-backforward"),
        QStringLiteral("button-reloadstop"),
        QStringLiteral("button-home"),
        QStringLiteral("locationbar"),
        QStringLiteral("button-downloads"),
        QStringLiteral("disablejs-icon"),
        QStringLiteral("button-tools")
    };
    ...
    if (!m_layoutIds.contains(QStringLiteral("locationbar"))) {
        m_layoutIds.append(QStringLiteral("locationbar"));
    }
...
{
    m_window->action(QStringLiteral("View/Stop"))->trigger();
}

void NavigationBar::reload()
{
    m_window->action(QStringLiteral("View/Reload"))->trigger();
}
