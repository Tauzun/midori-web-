...
{
    Settings settings;
    settings.beginGroup(QStringLiteral("Bookmarks"));
    m_showOnlyIconsInToolbar = settings.value(QStringLiteral("showOnlyIconsInToolbar"), false).toBool();
    m_showOnlyTextInToolbar = settings.value(QStringLiteral("showOnlyTextInToolbar"), false).toBool();
    settings.endGroup();
}
...
void Bookmarks::saveSettings()
{
    Settings settings;
    settings.beginGroup(QStringLiteral("Bookmarks"));
    settings.setValue(QStringLiteral("showOnlyIconsInToolbar"), m_showOnlyIconsInToolbar);
    settings.setValue(QStringLiteral("showOnlyTextInToolbar"), m_showOnlyTextInToolbar);
    settings.endGroup();
...
void Bookmarks::loadBookmarks()
{
    ...
    if (err.error != QJsonParseError::NoError || res.type() != QVariant::Map) {
        ...
        json = QJsonDocument::fromJson(QzTools::readAllFileByteContents(QStringLiteral(":data/bookmarks.json")), &err);
        const QVariant data = json.toVariant();

        Q_ASSERT(err.error == QJsonParseError::NoError);
        Q_ASSERT(data.type() == QVariant::Map);

        loadBookmarksFromMap(data.toMap().value(QStringLiteral("roots")).toMap());

        // Don't forget to save the bookmarks
        m_autoSaver->changeOccurred();
    }
    else {
        loadBookmarksFromMap(res.toMap().value(QStringLiteral("roots")).toMap());
    }
}
...
#define WRITE_FOLDER(name, mapName, folder) \
    QVariantMap mapName; \
    mapName.insert(QStringLiteral("children"), writeBookmarks(folder)); \
    mapName.insert(QStringLiteral("expanded"), folder->isExpanded()); \
    mapName.insert(QStringLiteral("expanded_sidebar"), folder->isSidebarExpanded()); \
    mapName.insert(QStringLiteral("name"), folder->title()); \
    mapName.insert(QStringLiteral("description"), folder->description()); \
    mapName.insert(QStringLiteral("type"), QStringLiteral("folder")); \
    bookmarksMap.insert(name, mapName);

    WRITE_FOLDER(QStringLiteral("bookmark_bar"), toolbarMap, m_folderToolbar)
    WRITE_FOLDER(QStringLiteral("bookmark_menu"), menuMap, m_folderMenu)
    WRITE_FOLDER(QStringLiteral("other"), unsortedMap, m_folderUnsorted)
#undef WRITE_FOLDER

    QVariantMap map;
    map.insert(QStringLiteral("version"), bookmarksVersion);
    map.insert(QStringLiteral("roots"), bookmarksMap);
...
#define READ_FOLDER(name, folder) \
    readBookmarks(map.value(name).toMap().value(QStringLiteral("children")).toList(), folder); \
    folder->setExpanded(map.value(name).toMap().value(QStringLiteral("expanded")).toBool()); \
    folder->setSidebarExpanded(map.value(name).toMap().value(QStringLiteral("expanded_sidebar")).toBool());

    READ_FOLDER(QStringLiteral("bookmark_bar"), m_folderToolbar)
    READ_FOLDER(QStringLiteral("bookmark_menu"), m_folderMenu)
    READ_FOLDER(QStringLiteral("other"), m_folderUnsorted)
#undef READ_FOLDER
}
...
void Bookmarks::readBookmarks(const QVariantList &list, BookmarkItem* parent)
{
    ...
    for (const QVariant &entry : list) {
        ...
        BookmarkItem::Type type = BookmarkItem::typeFromString(map.value(QStringLiteral("type")).toString());
        ...
        case BookmarkItem::Url:
            item->setUrl(QUrl::fromEncoded(map.value(QStringLiteral("url")).toByteArray()));
            item->setTitle(map.value(QStringLiteral("name")).toString());
            item->setDescription(map.value(QStringLiteral("description")).toString());
            item->setKeyword(map.value(QStringLiteral("keyword")).toString());
            item->setVisitCount(map.value(QStringLiteral("visit_count")).toInt());
            break;

        case BookmarkItem::Folder:
            item->setTitle(map.value(QStringLiteral("name")).toString());
            item->setDescription(map.value(QStringLiteral("description")).toString());
            item->setExpanded(map.value(QStringLiteral("expanded")).toBool());
            item->setSidebarExpanded(map.value(QStringLiteral("expanded_sidebar")).toBool());
            break;
        ...
        if (map.contains(QStringLiteral("children"))) {
            readBookmarks(map.value(QStringLiteral("children")).toList(), item);
        }
    }
}
...
QVariantList Bookmarks::writeBookmarks(BookmarkItem* parent)
{
    ...
    for (BookmarkItem* child : children) {
        ...
        switch (child->type()) {
        case BookmarkItem::Url:
            map.insert(QStringLiteral("url"), child->urlString());
            map.insert(QStringLiteral("name"), child->title());
            map.insert(QStringLiteral("description"), child->description());
            map.insert(QStringLiteral("keyword"), child->keyword());
            map.insert(QStringLiteral("visit_count"), child->visitCount());
            break;

        case BookmarkItem::Folder:
            map.insert(QStringLiteral("name"), child->title());
            map.insert(QStringLiteral("description"), child->description());
            map.insert(QStringLiteral("expanded"), child->isExpanded());
            map.insert(QStringLiteral("expanded_sidebar"), child->isSidebarExpanded());
            break;
        ...
        if (!child->children().isEmpty()) {
            map.insert(QStringLiteral("children"), writeBookmarks(child));
        }
