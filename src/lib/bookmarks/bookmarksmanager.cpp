...
void BookmarksManager::createContextMenu(const QPoint &pos)
{
    ...
    menu.addSeparator();
    QAction* actDelete = menu.addAction(QIcon(QStringLiteral(":icons/menu/edit-delete.svg")), tr("Delete"));
...
void BookmarksManager::addBookmark()
{
    ...
    item->setUrl(QUrl(QStringLiteral("http://")));
    addBookmark(item);
}
...
void BookmarksManager::descriptionEdited()
{
    ...
    // textChanged() is emitted also when text is changed programmatically
