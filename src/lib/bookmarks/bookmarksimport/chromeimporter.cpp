...
{
    m_path = QFileDialog::getOpenFileName(parent, BookmarksImporter::tr("Choose file..."), standardPath(), QStringLiteral("Bookmarks (Bookmarks)"));
    return m_path;
}
...
BookmarkItem* ChromeImporter::importBookmarks()
{
    ...
    QVariantMap rootMap = res.toMap().value(QStringLiteral("roots")).toMap();

    BookmarkItem* root = new BookmarkItem(BookmarkItem::Folder);
    root->setTitle(QStringLiteral("Chrome Import"));

    BookmarkItem* toolbar = new BookmarkItem(BookmarkItem::Folder, root);
    toolbar->setTitle(rootMap.value(QStringLiteral("bookmark_bar")).toMap().value(QStringLiteral("name")).toString());
    readBookmarks(rootMap.value(QStringLiteral("bookmark_bar")).toMap().value(QStringLiteral("children")).toList(), toolbar);

    BookmarkItem* other = new BookmarkItem(BookmarkItem::Folder, root);
    other->setTitle(rootMap.value(QStringLiteral("other")).toMap().value(QStringLiteral("name")).toString());
    readBookmarks(rootMap.value(QStringLiteral("other")).toMap().value(QStringLiteral("children")).toList(), other);

    BookmarkItem* synced = new BookmarkItem(BookmarkItem::Folder, root);
    synced->setTitle(rootMap.value(QStringLiteral("synced")).toMap().value(QStringLiteral("name")).toString());
    readBookmarks(rootMap.value(QStringLiteral("synced")).toMap().value(QStringLiteral("synced")).toList(), other);
...
void ChromeImporter::readBookmarks(const QVariantList &list, BookmarkItem* parent)
{
    ...
    for (const QVariant &entry : list) {
        ...
        const QString typeString = map.value(QStringLiteral("type")).toString();
