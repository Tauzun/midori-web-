...
BookmarkItem* HtmlImporter::importBookmarks()
{
    ...
    while (start > 0) {
        ...
        if (nearest == posOfFolder) {
            // Next is folder
            QRegularExpression rx(QStringLiteral("<dt><h3(.*)>(.*)</h3>"), QRegularExpression::InvertedGreedinessOption | QRegularExpression::DotMatchesEverythingOption);
            ...
            // Next is link
            QRegularExpression rx(QStringLiteral("<dt><a(.*)>(.*)</a>"), QRegularExpression::InvertedGreedinessOption | QRegularExpression::DotMatchesEverythingOption);
            QRegularExpressionMatch match = rx.match(string);

            QString arguments = match.captured(1);
            QString linkName = match.captured(2).trimmed();

            QRegularExpression rx2(QStringLiteral("href=\"(.*)\""), QRegularExpression::InvertedGreedinessOption | QRegularExpression::DotMatchesEverythingOption);
