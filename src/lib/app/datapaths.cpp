...
void DataPaths::setPortableVersion()
{
    ...
    d->m_paths[Config] = QStringList{appDir + QStringLiteral("/config")};
    d->m_paths[Cache] = QStringList{appDir + QStringLiteral("/cache")};
    d->m_paths[Profiles] = QStringList{appDir + QStringLiteral("/config/profiles")};
...
void DataPaths::init()
{
...
#ifdef MIDORI_PLUGIN_PATH
    m_paths[Plugins].append(QStringLiteral(MIDORI_PLUGIN_PATH));
#endif
    ...
    // If MIDORI_PLUGIN_PATH is set, only load plugins from there
    const QByteArray pluginPath = qgetenv("MIDORI_PLUGIN_PATH");
...
void DataPaths::initCurrentProfile(const QString &profilePath)
{
	...
	if (m_paths[Cache].isEmpty()) {
		...
		if (!QDir(m_paths[CurrentProfile].at(0) + QLatin1String("/cache")).exists()){
	...
	if (m_paths[Sessions].isEmpty()) {
		...
		if (!QDir(m_paths[CurrentProfile].at(0) + QLatin1String("/sessions")).exists()){
	...
	if (m_paths[Themes].isEmpty()) {
        	m_paths[Themes].append(m_paths[CurrentProfile].at(0) + QLatin1String("/themes"));
		if (!QDir(m_paths[CurrentProfile].at(0) + QLatin1String("/themes")).exists()){
    			dir.mkpath(m_paths[Themes].at(0));
		}
	}
...
void DataPaths::initAssetsIn(const QString &path)
{
    ...
    if (QDir(QCoreApplication::applicationDirPath() + QLatin1String("/../share/midori/themes/Noir")).exists()){
        m_paths[ThemesExtra].append(QCoreApplication::applicationDirPath() + QLatin1String("/../share/midori/themes"));
    } else {
        m_paths[ThemesExtra].append(path + QLatin1String("/themes"));
    }
#else
    m_paths[Themes].append(path + QLatin1String("/themes"));
#endif
    m_paths[Plugins].append(path + QLatin1String("/plug-ins"));
}
