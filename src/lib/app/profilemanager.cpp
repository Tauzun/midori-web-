...
void ProfileManager::initConfigDir()
{
    ...
    std::cout << "Midori Browser: Creating new profile directory" << std::endl;
...
void ProfileManager::updateProfile(const QString &current, const QString &profile)
{
    ...
    if (prof < Updater::Version(QStringLiteral("1.9.0"))) {
        std::cout << "Midori Browser: Using profile from QupZilla " << qPrintable(profile) << " is not supported!" << std::endl;
        return;
    }
...
void ProfileManager::copyDataToProfile()
{
    ...
    if (browseData.exists()) {
        ...
        QFile settings(profileDir.filePath(QStringLiteral("settings.ini")));
        if (settings.exists()) {
            const QString settingsBackup = QzTools::ensureUniqueFilename(profileDir.filePath(QStringLiteral("settings-backup.ini")));
            settings.copy(settingsBackup);
            settings.remove();
        }

        QFile sessionFile(profileDir.filePath(QStringLiteral("session.dat")));
        if (sessionFile.exists()) {
            QString oldVersion = QzTools::readAllFileContents(profileDir.filePath(QStringLiteral("version"))).trimmed();
            if (oldVersion.isEmpty()) {
                oldVersion = QStringLiteral("unknown-version");
            }
            const QString sessionBackup = QzTools::ensureUniqueFilename(profileDir.filePath(QStringLiteral("sessions/backup-%1.dat").arg(oldVersion)));
        ...
        QMessageBox::warning(0, QStringLiteral("Midori Browser: Incompatible profile version"), text);
    }
}
...
#if defined(Q_OS_WIN)
    const QString qzConfig = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/qupzilla");
#elif defined(Q_OS_MACOS)
    const QString qzConfig = QDir::homePath() + QLatin1String("/Library/Application Support/QupZilla");
#else // Unix
    const QString qzConfig = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QLatin1String("/qupzilla");
#endif
...
void ProfileManager::connectDatabase()
{
    ...
    if (db.tables().isEmpty()) {
        const QStringList statements = QzTools::readAllFileContents(QStringLiteral(":/data/browsedata.sql")).split(QLatin1Char(';'));
