    ...
    , m_hideNavigationTimer(nullptr)
{
    ...
    if (mApp->isPrivate()) {
        QzTools::setWmClass(QStringLiteral("Midori Browser (Private Window)"), this);
    }
    else {
        QzTools::setWmClass(QStringLiteral("Midori Browser"), this);
    }
...
void BrowserWindow::setupUi() {
    ...
    const QByteArray windowGeometry = settings.value(QStringLiteral("WindowGeometry")).toByteArray();

    const QStringList keys = {
        QStringLiteral("LocationBarWidth"),
        QStringLiteral("WebSearchBarWidth"),
        QStringLiteral("SideBarWidth"),
        QStringLiteral("WebViewWidth"),
    ...
    m_mainSplitter->setObjectName(QStringLiteral("sidebar-splitter"));
    ...
    m_statusBar->setObjectName(QStringLiteral("mainwindow-statusbar"));
    m_statusBar->setCursor(Qt::ArrowCursor);
    setStatusBar(m_statusBar);
    m_progressBar = new ProgressBar(m_statusBar);
    m_ipLabel = new QLabel(this);
    m_ipLabel->setObjectName(QStringLiteral("statusbar-ip-label"));
    ...
    int windowWidth = screenGeometry.height();
    int windowHeight = screenGeometry.width();
...
void BrowserWindow::setupMenu()
{
    ...
    QShortcut* reloadBypassCacheAction = new QShortcut(QKeySequence(QStringLiteral("Ctrl+F5")), this);
    QShortcut* reloadBypassCacheAction2 = new QShortcut(QKeySequence(QStringLiteral("Ctrl+Shift+R")), this);
    ...
    QShortcut* closeTabAction = new QShortcut(QKeySequence(QStringLiteral("Ctrl+W")), this);
    QShortcut* closeTabAction2 = new QShortcut(QKeySequence(QStringLiteral("Ctrl+F4")), this);
    ...
    QShortcut* reloadAction = new QShortcut(QKeySequence(QStringLiteral("Ctrl+R")), this);
    connect(reloadAction, &QShortcut::activated, this, &BrowserWindow::reload);

    QShortcut* openLocationAction = new QShortcut(QKeySequence(QStringLiteral("Alt+D")), this);
    connect(openLocationAction, &QShortcut::activated, this, &BrowserWindow::openLocation);

    QShortcut* inspectorAction = new QShortcut(QKeySequence(QStringLiteral("F12")), this);
...
QHash<QString, QVariant> BrowserWindow::saveUiState()
{
    ...
    state[QStringLiteral("LocationBarWidth")] = m_navigationToolbar->splitter()->sizes().at(0);
    state[QStringLiteral("WebSearchBarWidth")] = m_navigationToolbar->splitter()->sizes().at(1);
    state[QStringLiteral("SideBarWidth")] = m_sideBarWidth;
    state[QStringLiteral("WebViewWidth")] = m_webViewWidth;
    state[QStringLiteral("SideBar")] = m_sideBarManager->activeSideBar();
    return state;
}

void BrowserWindow::restoreUiState(const QHash<QString, QVariant> &state)
{
    const int locationBarWidth = state.value(QStringLiteral("LocationBarWidth"), 480).toInt();
    const int websearchBarWidth = state.value(QStringLiteral("WebSearchBarWidth"), 140).toInt();
    m_navigationToolbar->setSplitterSizes(locationBarWidth, websearchBarWidth);

    m_sideBarWidth = state.value(QStringLiteral("SideBarWidth"), 250).toInt();
    m_webViewWidth = state.value(QStringLiteral("WebViewWidth"), 2000).toInt();
    if (m_sideBar) {
        m_mainSplitter->setSizes({m_sideBarWidth, m_webViewWidth});
    }

    const QString activeSideBar = state.value(QStringLiteral("SideBar")).toString();
...
void BrowserWindow::loadSettings()
{
    ...
    bool showStatusBar = qzSettings->showStatusBar;
    bool showBookmarksToolbar = qzSettings->showBookmarksToolbar;
    bool showNavigationToolbar = qzSettings->showNavigationToolbar;
    bool showMenuBar = qzSettings->showMenuBar;
    // Make sure both menubar and navigationbar are not hidden
    if (!showNavigationToolbar) {
        showMenuBar = true;
        settings.setValue(QStringLiteral("Browser-View-Settings/showMenubar"), true);
    }
    m_useTabNumberShortcuts = qzSettings->useTabNumberShortcuts;
    m_useSpeedDialNumberShortcuts = qzSettings->useSpeedDialNumberShortcuts;
    m_useSingleKeyShortcuts = qzSettings->useSingleKeyShortcuts;
    QAction *quitAction = m_mainMenu->action(QStringLiteral("Standard/Quit"));
    if (qzSettings->closeAppWithCtrlQ) {
        quitAction->setShortcut(QzTools::actionShortcut(QKeySequence::Quit, QKeySequence(QStringLiteral("Ctrl+Q"))));
    } else {
...
void BrowserWindow::changeEncoding()
{
    if (QAction* action = qobject_cast<QAction*>(sender())) {
        ...
        Settings settings;
        settings.setValue(QStringLiteral("Web-Browser-Settings/DefaultEncoding"), encoding);
...
void BrowserWindow::saveSideBarSettings()
{
    ...
    Settings settings;
    settings.setValue(QStringLiteral("Browser-View-Settings/SideBar"), m_sideBarManager->activeSideBar());
}
...
void BrowserWindow::toggleShowMenubar()
{
    ...
    Settings settings;
    settings.setValue(QStringLiteral("Browser-View-Settings/showMenubar"), menuBar()->isVisible());
...
void BrowserWindow::toggleShowStatusBar()
{
    ...
    setUpdatesEnabled(true);
    Settings settings;
    settings.setValue(QStringLiteral("Browser-View-Settings/showStatusBar"), m_statusBar->isVisible());
}
...
void BrowserWindow::toggleShowBookmarksToolbar()
{
    ...
    setUpdatesEnabled(true);
    Settings settings;
    settings.setValue(QStringLiteral("Browser-View-Settings/showBookmarksToolbar"), m_bookmarksToolbar->isVisible());
    settings.setValue(QStringLiteral("Browser-View-Settings/instantBookmarksToolbar"), false);
}
...
void BrowserWindow::toggleShowNavigationToolbar()
{
    ...
    setUpdatesEnabled(true);
    Settings settings;
    settings.setValue(QStringLiteral("Browser-View-Settings/showNavigationToolbar"), m_navigationToolbar->isVisible());
...
void BrowserWindow::updateLoadingActions()
{
    ...
    action(QStringLiteral("View/Stop"))->setEnabled(isLoading);
    action(QStringLiteral("View/Reload"))->setEnabled(!isLoading);
...
void BrowserWindow::createToolbarsMenu(QMenu* menu)
{
    ...
    action->setCheckable(true);
    action->setChecked(qzSettings->showBookmarksToolbar);
...
void BrowserWindow::createEncodingMenu(QMenu* menu)
{
    ...
    createEncodingSubMenu(QStringLiteral("ISO"), isoCodecs, menu);
    createEncodingSubMenu(QStringLiteral("UTF"), utfCodecs, menu);
    createEncodingSubMenu(QStringLiteral("Windows"), windowsCodecs, menu);
    createEncodingSubMenu(QStringLiteral("Iscii"), isciiCodecs, menu);
    createEncodingSubMenu(QStringLiteral("IBM"), ibmCodecs, menu);
...
void BrowserWindow::searchOnPage()
{
    if (weView() && weView()->webTab()) {
        ...
        if (!searchText.contains(QLatin1Char('\n'))) {
...
{
    const QString fileTypes = QStringLiteral("%1(*.html *.htm *.shtml *.shtm *.xhtml);;"
                                      "%2(*.png *.jpg *.jpeg *.bmp *.gif *.svg *.tiff);;"
                                      "%3(*.md *.pdf *.txt);;"
                                      "%4(*.*)").arg(tr("HTML files"), tr("Image files"), tr("Text files"), tr("All files"));

    const QString filePath = QzTools::getOpenFileName(QStringLiteral("MainWindow-openFile"), this, tr("Open file..."), QDir::homePath(), fileTypes);
...
void BrowserWindow::keyPressEvent(QKeyEvent* event)
{
    ...
    case Qt::Key_News:
        action(QStringLiteral("Tools/RssReader"))->trigger();
        event->accept();
        break;

    case Qt::Key_Tools:
        action(QStringLiteral("Standard/Preferences"))->trigger();
        event->accept();
        break;
    ...
    case Qt::Key_I:
        if (event->modifiers() == Qt::ControlModifier) {
            action(QStringLiteral("Tools/SiteInfo"))->trigger();
            event->accept();
        }
        break;
    ...
    case Qt::Key_U:
        if (event->modifiers() == Qt::ControlModifier) {
            action(QStringLiteral("View/PageSource"))->trigger();
            event->accept();
        }
        break;
    ...
    case Qt::Key_F:
        if (event->modifiers() == Qt::ControlModifier) {
            action(QStringLiteral("Edit/Find"))->trigger();
            event->accept();
        }
        break;
...
void BrowserWindow::closeEvent(QCloseEvent* event)
{
    ...
    Settings settings;
    bool askOnClose = settings.value(QStringLiteral("Browser-Tabs-Settings/AskOnClosing"), true).toBool();
    ...
    if (askOnClose && m_tabWidget->normalTabsCount() > 1) {
        ...
        if (dialog.isChecked()) {
            settings.setValue(QStringLiteral("Browser-Tabs-Settings/AskOnClosing"), false);
        }
    }
