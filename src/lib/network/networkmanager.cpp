...
#include "schemehandlers/bhawkschemehandler.h"
    ...
    : QNetworkAccessManager(parent)
{
    ...
    mApp->webProfile()->installUrlSchemeHandler(QByteArrayLiteral("browser"), new BhawkSchemeHandler());
    mApp->webProfile()->installUrlSchemeHandler(QByteArrayLiteral("plug-in"), m_extensionScheme);
    WebPage::addSupportedScheme(QStringLiteral("browser"));
    WebPage::addSupportedScheme(QStringLiteral("plug-in"));
...
bool NetworkManager::rejectedSslErrors(QString host) {

    if (certErrorTrigger) {

        if (m_rejectedSslErrors.contains(host)) {
...
bool NetworkManager::ignoredSslErrors(QString host) {

    if (certErrorTrigger) {

        if (m_ignoredSslErrors.contains(host)) {
...
bool NetworkManager::certificateError(const QWebEngineCertificateError &error, QWidget *parent)
{
    ...
    if (certErrorTrigger) {
        ...
        if (NetworkManager::rejectedSslErrors(host) && m_rejectedSslErrors.value(host) == error.error()) {
            return false;
        }

        if (NetworkManager::ignoredSslErrors(host) && m_ignoredSslErrors.value(host) == error.error()) {
            return true; // Why? ssl error STILL exists, ignored or not.
        }
    }

    certErrorTrigger = true;
    ...
    QString message = QStringLiteral("<b>%1</b><p>%2</p><ul><li>%3</li></ul><p>%4</p>").arg(title, text1, error.errorDescription(), text2);
    ...
    switch (dialog.result()) {
        ...
        case SslErrorDialog::Yes:
            // Falkon want to support Permanent exceptions here. This will never happen in bhawk.
...
void NetworkManager::registerSchemes() {
    ...
    QWebEngineUrlScheme bhawkScheme("browser"); // Was "falkon"
    bhawkScheme.setFlags(QWebEngineUrlScheme::CorsEnabled | QWebEngineUrlScheme::LocalScheme | QWebEngineUrlScheme::ContentSecurityPolicyIgnored);
    bhawkScheme.setSyntax(QWebEngineUrlScheme::Syntax::Path);
    QWebEngineUrlScheme::registerScheme(bhawkScheme);

    QWebEngineUrlScheme extensionScheme("plug-in");
    extensionScheme.setFlags(QWebEngineUrlScheme::CorsEnabled | QWebEngineUrlScheme::LocalScheme | QWebEngineUrlScheme::ContentSecurityPolicyIgnored);
