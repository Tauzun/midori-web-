...
PIM_Handler::PIM_Handler(const QString &sPath, QObject* parent)
    : QObject(parent)
    , m_settingsFile(sPath + QLatin1String("/plug-ins.ini"))
    , m_loaded(false)
{
}
...
bool PIM_Handler::keyPress(WebView* view, QKeyEvent* event)
{
    ...
    QString source = QLatin1String("var inputs = document.getElementsByTagName('input');"
                          "var table = %1;"
    ...
    view->page()->runJavaScript(source.arg(matchingJsTable()), WebPage::jsAppWorldSafe);
...
void PIM_Handler::pimInsert()
{
    ...
    QString source = QLatin1String("var e = document.elementFromPoint(%1, %2);"
                          "if (e) {"
    ...
    m_view->page()->runJavaScript(source, WebPage::jsAppWorldSafe);
}
...
void PIM_Handler::pageLoadFinished()
{
    ...
    page->runJavaScript(source.arg(matchingJsTable()), WebPage::jsAppWorldSafe);
}
...
QString PIM_Handler::matchingJsTable() const
{
    ...
    while (i.hasNext()) {
        ...
        for (const QString &value : ivalues) {
            ...
            values.append(QStringLiteral("\"%1\":\"%2\",").arg(value, key));
        }
    }
    ...
    return QStringLiteral("{ %1 }").arg(values);
}
