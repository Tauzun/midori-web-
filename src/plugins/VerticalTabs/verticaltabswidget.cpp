    ...
    , m_window(window)
{
    ...
    buttonAddTab->setObjectName(QStringLiteral("verticaltabs-button-addtab"));
    buttonAddTab->setAutoRaise(true);
    buttonAddTab->setFocusPolicy(Qt::NoFocus);
    buttonAddTab->setToolTip(tr("New Tab"));
    buttonAddTab->setIcon(QIcon(QStringLiteral(":icons/menu/list-add.svg")));
...
void VerticalTabsWidget::updateGroupMenu()
{
    ...
    for (int i = 0; i < m_window->tabWidget()->count(); ++i) {
        ...
        if (tab->url().toString(QUrl::RemoveFragment) == QLatin1String("plug-in://verticaltabs/group")) {
    ...
    m_groupMenu->addAction(tr("Add New Group..."), this, [this]() {
        m_window->tabWidget()->addView(QUrl(QStringLiteral("plug-in://verticaltabs/group")), Qz::NT_SelectedTab);
    });
}
