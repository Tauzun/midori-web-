...
void GM_SettingsListDelegate::paint(QPainter* painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    const int newPointSize = titleFont.pointSize() + 1;

    if (newPointSize > 1) {
        titleFont.setPointSize(newPointSize);
    } else {
    ...
    QRect styleCheckBoxRect = style->subElementRect(QStyle::SE_CheckBoxIndicator, &opt2, w);
    opt2.rect = QRect(leftPosition, checkboxYPos, styleCheckBoxRect.width(), styleCheckBoxRect.height());
    style->drawPrimitive(QStyle::PE_IndicatorCheckBox, &opt2, painter, w);
