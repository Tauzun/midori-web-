    ...
    , m_manager(manager)
{
    ...
    connect(ui->link_1, &ClickableLabel::clicked,
            this, &GM_Settings::openUserJs);
    connect(ui->link_2, &ClickableLabel::clicked,
            this, &GM_Settings::greaseSpot);
...
{
    mApp->addNewTab(QUrl(QStringLiteral("https://openuserjs.org")));
    close();
}

void GM_Settings::greaseSpot()
{
    mApp->addNewTab(QUrl(QStringLiteral("https://wiki.greasespot.net/Greasemonkey_Manual:API")));
    close();
}
...
void GM_Settings::newScript()
{
    ...
    const QString script = QLatin1String("// ==UserScript==\n"
                                ...
                                "// @license      CC Attribution IPL 4.0; https://creativecommons.org/licenses/by/4.0/\n"
                                "// @copyright    202?, Your Name\n"
                                "// @version      0.1\n"
                                "// @run-at       document-end\n"
                                "// @include      http*\n"
                                "// @exclude      browser*\n"
    ...
    const QString fileName = QStringLiteral("%1/%2.user.js").arg(m_manager->scriptsDirectory(), QzTools::filterCharsFromFilename(name));
