...
{
    m_name.clear();
    m_namespace = QStringLiteral("org.browser.bhawk");
    m_description.clear();
    m_version.clear();
    m_include.clear();
    m_exclude.clear();
    ...
    if (m_include.isEmpty()) {
        m_include.append(QStringLiteral("*"));
    }
    ...
    m_script = QStringLiteral("(function (GM) {\n%1\n%2\n%3\n}(window.GM = window.GM || {}));").arg(gmValues, m_manager->requireScripts(m_require), fileData);
    m_valid = true;
