...
#include <PyBhawk/pybhawk_python.h>

extern "C" PyObject *PyInit_PyBhawk();
...
static void set_path(const QStringList &scriptPaths)
{
    ...
    qputenv("PYTHONPATH", paths.join(QLatin1Char(':')).toLocal8Bit());
}
...
static State init()
{
    ...
    if (PyImport_AppendInittab("Bhawk", PyInit_PyBhawk) != 0) {
        PyErr_Print();
        qWarning() << "Failed to initialize Bhawk module!";
...
void pybhawk_register_plugin(PluginInterface *plugin)
{
    pluginInterface = plugin;
}

void *pybhawk_load_plugin(const QString &name)
{
    QString fullPath;
    ...
    plugin->pluginId = QStringLiteral("python:%1").arg(QFileInfo(name).fileName());
    plugin->pluginPath = fullPath;
    plugin->pluginSpec = Plugins::createSpec(DesktopFile(fullPath + QStringLiteral("/metadata.desktop")));
    return plugin;
}

void pybhawk_init_plugin(Plugins::Plugin *plugin)
{
    ...
    if (!pluginInterface) {
        qWarning() << "No plugin registered! Bhawk.registerPlugin() must be called from script.";
        return;
    }
