...
{
    if (argc != 2) {
        std::cerr << "pybhawktestrunner [test.py]" << std::endl;
        return 1;
    }

    QzTools::removeRecursively(QDir::tempPath() + QStringLiteral("/Bhawk-test"));
    MainApplication::setTestModeEnabled(true);
    MainApplication app(argc, argv);

#if defined(Q_OS_MACOS)
    const QString suffix = QStringLiteral("dylib");
    ...
    QLibrary library(QFINDTESTDATA("plug-ins/PyBhawk." + suffix));
