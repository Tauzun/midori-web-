...
{
    QString appendString = QLatin1String(":");
...
void TabItem::updateIcon()
{
    ...
    if (!m_webTab->isLoading()) {
        if (!m_webTab->isPinned()) {
            if (m_webTab->isMuted()) {
                setIcon(0, QIcon(QStringLiteral(":icons/other/audiomuted.svg")));
            }
            else if (!m_webTab->isMuted() && m_webTab->webView()->page()->recentlyAudible()) {
                setIcon(0, QIcon(QStringLiteral(":icons/other/audioplaying.svg")));
            }
            else {
...
#define MIMETYPE QLatin1String("application/bhawk.tabs")
