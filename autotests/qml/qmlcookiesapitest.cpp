...
void QmlCookiesApiTest::testCookieAdditionRemoval()
{
    ...
    m_testHelper.evaluate("Bhawk.Cookies.set({"
                     "    name: 'Example',"
                     "    url: '.example.com',"
    ...
    QObject *object = m_testHelper.evaluateQObject("Bhawk.Cookies");
    QVERIFY(object);
    ...
    m_testHelper.evaluate("Bhawk.Cookies.remove({"
                     "    name: 'Hello',"
                     "    url: '.mydomain.com',"
                     "})");
...
void QmlCookiesApiTest::testCookieGet()
{
    ...
    QObject *mangoCookie = m_testHelper.evaluateQObject("Bhawk.Cookies.get({"
                     "    name: 'Mango',"
    ...
    QList<QVariant> mangoCookies = m_testHelper.evaluate("Bhawk.Cookies.getAll({name: 'Mango'})").toVariant().toList();
    QCOMPARE(mangoCookies.length(), 2);
}

BHAWKTEST_MAIN(QmlCookiesApiTest)
