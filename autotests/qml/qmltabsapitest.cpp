...
{
    // Tab Insertion
    QObject *qmlTabsObject = m_testHelper.evaluateQObject("Bhawk.Tabs");
    QVERIFY(qmlTabsObject);
    QSignalSpy qmlTabsInsertedSpy(qmlTabsObject, SIGNAL(tabInserted(QVariantMap)));
    m_testHelper.evaluate("Bhawk.Tabs.addTab({"
    ...
    QObject *qmlTabObject1 = m_testHelper.evaluateQObject("Bhawk.Tabs.get({index: 0})");
    ...
    bool pinnedTab = m_testHelper.evaluate("Bhawk.Tabs.pinTab({index: 1})").toBool();
    QVERIFY(pinnedTab);
    QObject *qmlTabObject2 = m_testHelper.evaluateQObject("Bhawk.Tabs.get({index: 0})");
    ...
    bool unpinnedTab = m_testHelper.evaluate("Bhawk.Tabs.unpinTab({index: 0})").toBool();
    QVERIFY(unpinnedTab);
    QObject *qmlTabObject3 = m_testHelper.evaluateQObject("Bhawk.Tabs.get({index: 0})");
    ...
    m_testHelper.evaluate("Bhawk.Tabs.nextTab()");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 1);
    m_testHelper.evaluate("Bhawk.Tabs.nextTab()");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 0);
    m_testHelper.evaluate("Bhawk.Tabs.previousTab()");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 1);
    m_testHelper.evaluate("Bhawk.Tabs.previousTab()");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 0);
    m_testHelper.evaluate("Bhawk.Tabs.setCurrentIndex({index: 1})");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 1);
    m_testHelper.evaluate("Bhawk.Tabs.setCurrentIndex({index: 0})");
    QCOMPARE(mApp->getWindow()->tabWidget()->currentIndex(), 0);

    // Move Tab
    QSignalSpy qmlTabsMovedSpy(qmlTabsObject, SIGNAL(tabMoved(QVariantMap)));
    m_testHelper.evaluate("Bhawk.Tabs.moveTab({from: 0, to:1, windowId: 0})");
    ...
    m_testHelper.evaluate("Bhawk.Tabs.closeTab({index: 0})");
    QCOMPARE(qmlTabsRemovedSpy.count(), 1);
    QCOMPARE(mApp->getWindow()->tabCount(), 1);
}

BHAWKTEST_MAIN(QmlTabsApiTest)
