...
{
    QObject *currentWindowObject = m_testHelper.evaluateQObject("Bhawk.Windows.getCurrent()");
    ...
    QObject *windowObject = m_testHelper.evaluateQObject("Bhawk.Windows");
    ...
    QObject *newQmlWindow = m_testHelper.evaluateQObject("Bhawk.Windows.create({})");
    ...
    int qmlWindowCount = m_testHelper.evaluate("Bhawk.Windows.getAll().length").toInt();
    ...
    m_testHelper.evaluate(QString("Bhawk.Windows.remove(%1)").arg(newQmlWindowId));
    QTRY_COMPARE(qmlWindowRemovedSignal.count(), 1);
}

BHAWKTEST_MAIN(QmlWindowsApiTest)
