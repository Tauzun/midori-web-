...
void QmlHistoryApiTest::testAddition()
{
    ...
    m_testHelper.evaluate("Bhawk.History.addUrl({"
...
{
    int visits = m_testHelper.evaluate("Bhawk.History.getVisits('https://sample.com')").toInt();
    QCOMPARE(visits, 1);
    QSignalSpy historySpy(mApp->history(), &History::historyEntryEdited);
    mApp->history()->addHistoryEntry(QUrl("https://sample.com"), "Sample Domain");
    QTRY_COMPARE(historySpy.count(), 1);
    visits = m_testHelper.evaluate("Bhawk.History.getVisits('https://sample.com')").toInt();
    QCOMPARE(visits, 2);
}
...
BHAWKTEST_MAIN(QmlHistoryApiTest)
