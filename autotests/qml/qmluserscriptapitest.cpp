...
{
    int count = m_testHelper.evaluate("Bhawk.UserScripts.count").toInt();
    QCOMPARE(count, mApp->webProfile()->scripts()->count());
}

void QmlUserScriptApiTest::testSize()
{
    int size = m_testHelper.evaluate("Bhawk.UserScripts.size").toInt();
    QCOMPARE(size, mApp->webProfile()->scripts()->size());
}

void QmlUserScriptApiTest::testEmpty()
{
    bool empty = m_testHelper.evaluate("Bhawk.UserScripts.empty").toBool();
...
void QmlUserScriptApiTest::testContains()
{
    ...
    QObject *object = m_testHelper.evaluateQObject("Bhawk.UserScripts");
...
{
    int initialCount = m_testHelper.evaluate("Bhawk.UserScripts.count").toInt();
    QObject *object = m_testHelper.evaluateQObject("Bhawk.UserScripts");
    QmlUserScripts *userScripts = dynamic_cast<QmlUserScripts*>(object);
    QVERIFY(userScripts);
    QmlUserScript *userScript = new QmlUserScript();
    ...
    int finalCount = m_testHelper.evaluate("Bhawk.UserScripts.count").toInt();
    QCOMPARE(finalCount, initialCount + 1);

    userScripts->remove(userScript);

    int ultimateCount = m_testHelper.evaluate("Bhawk.UserScripts.count").toInt();
    QCOMPARE(ultimateCount, initialCount);
}

BHAWKTEST_MAIN(QmlUserScriptApiTest)
