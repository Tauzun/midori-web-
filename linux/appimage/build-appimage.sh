#!/bin/sh
###############################################################################
# Compile Midori source and pack it as Appimage.
###############################################################################
set -e
...
${BLD1}--sourcedir=${BLD0}
          ...
          Assuming this script is located in ${ITL1}midori/linux${ITL0},
          otherwise you must specify the path to
          Midori source directory.

               ${UDR1}example:--sourcedir="/home/build/midori"${UDR0}
...
${BLD1}--update-source | -U${BLD0}
          ...
          Fetches the information from Midori online git repository
...
printf "Compiling Midori with the following settings:\n"
printConf
...
pushd bundle_build_dir/lib/plugins/midori
...
CRYPTONEEDED="$(ldd 'bin/midori'| grep libcrypto | sed 's/.*=>//;s/(.*//')"
...
cp ../linux/applications/org.astian.midori.desktop bundle_build_dir
cp ../linux/pixmaps/midori.png bundle_build_dir
ln -s midori.png bundle_build_dir/.DirIcon

pushd bundle_build_dir
mv bin/midori ./ && rm -fr bin
chrpath --replace '$ORIGIN' lib/libMidoriPrivate.so.3.*
chrpath --replace '$ORIGIN/lib' midori
...
MIDORI_DIR="\$(dirname "\$(readlink -f "\$0")")"

XDG_DATA_DIRS="\${MIDORI_DIR}/share:\${XDG_DATA_DIRS}"
MIDORI_PLUGIN_PATH="\${MIDORI_DIR}/lib/plugins/midori"
export XDG_DATA_DIRS MIDORI_PLUGIN_PATH
...
mksquashfs bundle_build_dir midori.squashfs -root-owned -noappend

cat "${RUNTIME_BINARY}" >bin/Midori.AppImage
cat midori.squashfs >>bin/Midori.AppImage
chmod a+x bin/Midori.AppImage
}
...
printf "Please install ${UDR1}$0${UDR0} in „${BLD1}scripts${BLD0}“ ${ITL1}(a sub folder in Midori source directory)${ITL0},
...
if [[ $? == 0 ]] && [[ -x bin/Midori.AppImage ]]; then
 printf "\\033c"
 printf "Done!\nThe compiled files are in "${PWD}"/bin\n"
 if [ ! -z "$OUT_DIR" ]; then
     cp bin/Midori.AppImage "$OUT_DIR"
 fi
fi
